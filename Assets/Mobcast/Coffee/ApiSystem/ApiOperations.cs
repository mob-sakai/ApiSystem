...
namespace Mobcast.Coffee.Api
{
	...
	public class ApiOperation : CustomYieldInstruction, IDisposable
	{
		...
		public override bool keepWaiting { get { return m_webRequest != null; } }

		public ApiOperation(IRequestPacket apiRequest, ApiRequestMeta requestMeta, byte[] data)
		{
			m_RequestPacket = apiRequest;
			m_RequestMeta = requestMeta;
			m_Data = data;
			retry = m_RequestPacket.retryCount;

			Debug.LogFormat ("[ApiOperation] Api通信をリクエスト<{0}>  : url = {1}", apiRequest.GetType(), apiRequest.url);
		}
		...
		public bool Update()
		{
			...
			if (0 < m_RetryDelay)
			{
				m_RetryDelay -= Time.unscaledDeltaTime;
				return true;
			}
			...
			if (m_webRequest == null) {
				...
				m_webRequest.timeout = m_RequestPacket.timeout;
				#endif
			...
			// 通信中...
			if (!m_webRequest.isDone && (Time.realtimeSinceStartup - m_Time) < m_RequestPacket.timeout)
			{
				return true;
			}
			...
			if (!success && 0 < retry) {
				Debug.LogWarningFormat ("[ApiOperation] Api通信をリトライ(リトライ = {0}, 経過時間 = {1})<{2}> : url = {3}\nstatus = {4}, error = {5}"
					, retry
					, Time.realtimeSinceStartup - m_Time
					, m_RequestPacket.GetType()
					, m_webRequest.url
